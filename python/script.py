...
async def check(ctx):
    ...
    try:
        ...
        response = requests.get(
            BACKEND_API_ENDPOINT + "/decode",
            params={"email": user_encoded_email, "secret": "H@rkirat"},
        )
        ...
        if response.status_code == 200:
            ...
            if check_discord_link(str(ctx.author.id), check_discord_link):
                print(
                    "Your Discord ID has already been linked with your email. "
                    + user_decrypted_email
                )
                return
            ...
            if True:
                ...
                print(
                    "Your Discord ID has been linked with your email. "
                    + user_decrypted_email
                )
